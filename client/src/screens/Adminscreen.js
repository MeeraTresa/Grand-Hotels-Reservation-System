...
export function Rooms() {
  ...
  useEffect(async () => {
    try {
      setloading(true);
      const data = await (
        await axios.get("http://LB-23492172.us-west-1.elb.amazonaws.com:5000/api/rooms/getallrooms")
      ).data;
      setrooms(data);
      setloading(false);
    } catch (error) {
...
export function Users(){
  ...
  useEffect(async() => {
    ...
    try {
      const data = await (await axios.get('http://LB-23492172.us-west-1.elb.amazonaws.com:5000/api/users/getallusers')).data
      setusers(data)
      setloading(false)
    } catch (error) {
      console.log(error)
